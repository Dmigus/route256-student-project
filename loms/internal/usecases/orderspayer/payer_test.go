...
func TestOrdersPayerPositive(t *testing.T) {
	t.Parallel()
	...
	helper.stocksRepoMock.Expect(minimock.AnyContext, items).Return(nil)
...
func TestOrdersPayerErrors(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			mockSetup: func(helper testHelper) {
				...
				order.Items = items
				helper.orderLoadRepoMock.Expect(minimock.AnyContext, 1234).Return(order, nil)
				helper.stocksRepoMock.Expect(minimock.AnyContext, items).Return(errorToThrow)
			},
			args: args{
				ctx:     context.Background(),
				orderId: 1234,
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			helper := newTestHelper(t)
			tt.mockSetup(helper)
			err := helper.payer.Pay(tt.args.ctx, tt.args.orderId)
